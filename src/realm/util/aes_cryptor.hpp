...
public:
    ...
    bool read(File fd, off_t pos, char* dst, size_t size);
    void write(File fd, off_t pos, const char* src, size_t size) noexcept;
...
private:
    ...
    iv_table& get_iv_table(File fd, off_t data_pos) noexcept;
};
...
struct SharedFileInfo {
    ...
    SharedFileInfo(const uint8_t* key, File file_descriptor);
};
}
}
